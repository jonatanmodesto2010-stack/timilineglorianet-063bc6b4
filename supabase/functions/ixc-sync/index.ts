...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // List filiais from IXC API
    if (action === 'list_filiais') {
      ...
      if (!integration?.api_url || !integration?.api_token) {
        return new Response(JSON.stringify({ filiais: [] }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      const token = encodeIxcToken(integration.api_token);
      const filiais = await fetchAllIxcRecords(integration.api_url, token, 'filial');
      const result = filiais.map((f: any) => ({
        id: String(f.id),
        name: f.razao || f.fantasia || `Filial ${f.id}`,
      }));

      return new Response(JSON.stringify({ filiais: result }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    // Get client IDs by filial from IXC API
    if (action === 'clients_by_filial') {
      const { organization_id, filial_id } = body;
      if (!organization_id || !filial_id) {
        return new Response(JSON.stringify({ error: 'organization_id e filial_id são obrigatórios' }), {
          status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (!integration?.api_url || !integration?.api_token) {
        return new Response(JSON.stringify({ client_ids: [] }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      const token = encodeIxcToken(integration.api_token);
      const clients = await fetchAllIxcRecords(integration.api_url, token, 'cliente', {
        qtype: 'cliente.id_filial',
        query: filial_id,
        oper: '=',
      });
      const clientIds = clients.map((c: any) => String(c.id));

      // Also update ixc_filial_id in DB for these clients
      if (clientIds.length > 0) {
        const filialName = body.filial_name || `Filial ${filial_id}`;
        ...
        if (timelines && timelines.length > 0) {
          for (let i = 0; i < timelines.length; i += 200) {
            const chunk = timelines.slice(i, i + 200);
            const ids = chunk.map(t => t.id);
            await supabase
              .from('client_timelines')
              .update({ ixc_filial_id: filial_id, ixc_filial_name: filialName })
              .in('id', ids);
          }
        }
      }

      return new Response(JSON.stringify({ client_ids: clientIds, total: clientIds.length }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
        ...
        if (action === 'sync' || action === 'cron' || action === 'sync_all' || action === 'sync_clients' || action === 'check_blocked') {
          ...
          for (const client of clients) {
            ...
            if (!isClientActive) {
              ...
              isActive = contract.active && !contract.blocked;
